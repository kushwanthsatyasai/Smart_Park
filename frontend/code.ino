...
// Updated ESP32-CAM AI Thinker Pin Mapping from JSON
#define PWDN_GPIO_NUM     32   // Updated based on JSON
#define RESET_GPIO_NUM    -1   // GPIO_NUM_NC from JSON
...
const int SD_CS = 13;  // Changed from 5 which conflicts with camera pin
WebServer server(80);

// Define User LED from JSON
const int USER_LED = 33;
const int USER_LED_ON = LOW;
...
void setup() {
  ...
  digitalWrite(USER_LED, !USER_LED_ON); // Start with LED off
  ...
  config.xclk_freq_hz = 20000000;   // Updated to 20MHz as per JSON
  config.pixel_format = PIXFORMAT_JPEG;
  config.frame_size = FRAMESIZE_QVGA;
  config.jpeg_quality = 12;
  config.fb_count = 2;              // Updated to 2 as per JSON
  config.fb_location = CAMERA_FB_IN_PSRAM; // Store framebuffers in PSRAM
  ...
  if (MDNS.begin("esp32cam")) {
    ...
    Serial.println("You can now connect to http://esp32cam.local");
  } else {
  ...
  Serial.println("Use one of these in your browser:");
  Serial.print("http://");
  Serial.println(WiFi.localIP());
  Serial.println("http://esp32cam.local (if mDNS works on your network)");
  Serial.println("==============================\n");
}
...
bool processQRCode(camera_fb_t *fb) {
  ...
  // Blink LED when QR code is detected
  digitalWrite(USER_LED, USER_LED_ON);
  delay(100);
  digitalWrite(USER_LED, !USER_LED_ON);
...
void verifyAndOperateGate(String qrData) {
  ...
  if (httpResponseCode == 200) {
    ...
    if (!error && doc["verified"] == true) {
      ...
      // Flash USER_LED to indicate successful verification
      for (int i = 0; i < 3; i++) {
        digitalWrite(USER_LED, USER_LED_ON);
        delay(200);
        digitalWrite(USER_LED, !USER_LED_ON);
...
void handleRoot() {
  ...
  html += "  };";
  html += "}";  // Close the function properly
  html += "</script>";
...
void handleStream() {
  ...
  html += "h1 { color: #333; }";
  html += ".button { display: inline-block; padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px; margin: 10px 0; }";
  html += "img { max-width: 640px; margin: 20px auto; display: block; }";
  html += "</style>";
  html += "</head><body>";
  
  html += "<h1>ESP32-CAM Live Stream</h1>";
  html += "<p><a href='/' class='button'>Back to Home</a></p>";
  html += "<div id='stream'><img id='streamImg' src='/capture'></div>";
  
  html += "<script>";
  html += "function updateImage() {";
  html += "  var img = document.getElementById('streamImg');";
  html += "  img.src = '/capture?' + new Date().getTime();";
  html += "  setTimeout(updateImage, 500);";
  html += "}";
...
void handleStatus() {
  // Create a JSON response with system status
  String response = "{";
  ...
  response += "\"camera_status\":\"" + String(esp_camera_status()) + "\"";
  response += "}";
